...
#define UNITY_VERTEX_INPUT_GPUANIMATION float2 boneIds  : TEXCOORD2; float2 boneInfluences : TEXCOORD3; 
...
inline float4 CalculateSkinPostionOS(float3 animationTextureCoords, float2 boneIds, float2 boneInfluences,float4 positionOS)
{
	// We interpolate between two matrices
	float4x4 frame0_BoneMatrix0 = CreateMatrix(animationTextureCoords.x, boneIds.x);
	float4x4 frame0_BoneMatrix1 = CreateMatrix(animationTextureCoords.y, boneIds.x);
    ...
    //posWorld=mul(mul(localToWorld*BoneMatrix),origPositionObject)
    //calulatePositionObject=mul(BoneMatrix*origPositionObject)
	//return frame0_BoneMatrix * boneInfluences.x + frame1_BoneMatrix * boneInfluences.y;
	return mul(frame0_BoneMatrix , positionOS) * boneInfluences.x + mul(frame1_BoneMatrix , positionOS) * boneInfluences.y;
}